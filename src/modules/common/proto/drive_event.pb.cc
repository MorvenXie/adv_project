...
DriveEvent::DriveEvent(const DriveEvent& from)
  ...
  DriveEvent* const _this = this; (void)_this;
  new (&_impl_) Impl_{
  ...
  if (from._internal_has_location()) {
    _this->_impl_.location_ = new ::apollo::localization::Pose(*from._impl_.location_);
  }
  _this->_impl_.is_reportable_ = from._impl_.is_reportable_;
...
void DriveEvent::MergeImpl(::PROTOBUF_NAMESPACE_ID::Message& to_msg, const ::PROTOBUF_NAMESPACE_ID::Message& from_msg) {
  auto* const _this = static_cast<DriveEvent*>(&to_msg);
  auto& from = static_cast<const DriveEvent&>(from_msg);
  ...
  if (cached_has_bits & 0x0000000fu) {
    ...
    if (cached_has_bits & 0x00000004u) {
      _this->_internal_mutable_location()->::apollo::localization::Pose::MergeFrom(
