...
SpeedControls::SpeedControls(const SpeedControls& from)
  : ::PROTOBUF_NAMESPACE_ID::Message() {
  SpeedControls* const _this = this; (void)_this;
  new (&_impl_) Impl_{
...
void SpeedControls::MergeImpl(::PROTOBUF_NAMESPACE_ID::Message& to_msg, const ::PROTOBUF_NAMESPACE_ID::Message& from_msg) {
  auto* const _this = static_cast<SpeedControls*>(&to_msg);
  auto& from = static_cast<const SpeedControls&>(from_msg);
  // @@protoc_insertion_point(class_specific_merge_from_start:apollo.hdmap.SpeedControls)
  GOOGLE_DCHECK_NE(&from, _this);
  uint32_t cached_has_bits = 0;
  (void) cached_has_bits;

  _this->_impl_.speed_control_.MergeFrom(from._impl_.speed_control_);
  _this->_internal_metadata_.MergeFrom<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(from._internal_metadata_);
}
