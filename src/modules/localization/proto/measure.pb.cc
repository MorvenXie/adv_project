...
IntegMeasure::IntegMeasure(const IntegMeasure& from)
  ...
  IntegMeasure* const _this = this; (void)_this;
  new (&_impl_) Impl_{
...
void IntegMeasure::MergeImpl(::PROTOBUF_NAMESPACE_ID::Message& to_msg, const ::PROTOBUF_NAMESPACE_ID::Message& from_msg) {
  auto* const _this = static_cast<IntegMeasure*>(&to_msg);
  auto& from = static_cast<const IntegMeasure&>(from_msg);
  // @@protoc_insertion_point(class_specific_merge_from_start:apollo.localization.IntegMeasure)
  GOOGLE_DCHECK_NE(&from, _this);
  uint32_t cached_has_bits = 0;
  (void) cached_has_bits;

  _this->_impl_.measure_covar_.MergeFrom(from._impl_.measure_covar_);
  ...
  if (cached_has_bits & 0x000000ffu) {
    ...
    if (cached_has_bits & 0x00000008u) {
      _this->_impl_.measure_type_ = from._impl_.measure_type_;
    }
    if (cached_has_bits & 0x00000010u) {
    ...
    if (cached_has_bits & 0x00000080u) {
      _this->_impl_.is_have_variance_ = from._impl_.is_have_variance_;
    }
    _this->_impl_._has_bits_[0] |= cached_has_bits;
  }
  if (cached_has_bits & 0x00000100u) {
    _this->_internal_set_is_gnss_double_antenna(from._internal_is_gnss_double_antenna());
  }
